...
abstract class MultiSenseWordEmbeddingModel(val opts: EmbeddingOpts) extends Parameters {
  ...
  // default value is 12
  protected val adaGradDelta = opts.delta.value
  ...
  //
  val storeInBinary = opts.binary.value
  // default is true
  ...
  protected val createClusterlambda = opts.createClusterLambda.value
  ...
  protected var ncluster: Array[Int] = null
  // holds the information for # of cluster
  ...
  def buildVocab(minFreq: Int = 5): Unit = {
    ...
    if (loadVocabFilename.size == 0) {
      ...
      while (corpusLineItr.hasNext) {
        ...
        line.stripLineEnd.split(' ').drop(1).foreach(word => vocab.addWordToVocab(word.toLowerCase()))
      }
    }
  ...
  def store(): Unit = {
    ...
    out = new OutputStreamWriter(new GZIPOutputStream(new BufferedOutputStream(new FileOutputStream("community_histo.gz"))), encoding)
    for (v <- 0 until V) {
      val C = if (learnMultiVec(v)) ncluster(v) else 1
      out.write(vocab.getWord(v) + " " + C)
      out.write("\n")
      out.flush()
      for (s <- 0 until C) {
        out.write(vocab.communityWordSense(v, s))
        out.write("\n")
        out.flush()
      }
    }
    out.close()
    println("Done writing community")
  }
