...
class VocabBuilder(vocab_hash_size: Int = 20e6.toInt, sampling_table_size: Int = 1e8.toInt, load_factor: Double = 0.7) {
  ...
  // vocab_hash stores pointer to the vocab array
  private var vocab = new Array[vocab_word](vocab_max_size)
  //  vocab array holds the (word, cnt) pairs. In future, huffman encoding will be added.
  private var sampling_table = new Array[Int](sampling_table_size)
  private var sub_sampling_table = new Array[Double](vocab_size)

  // LINAN: word, sense, community count tuple
  private var community_counts = Array.ofDim[Int](vocab_max_size, 10, 10)

  (0 until vocab_hash_size).foreach(i => vocab_hash(i) = -1)
  (0 until vocab_max_size).foreach(i => vocab(i) = null)

  def communityWordSense(word: Int, sense: Int): String = {
    community_counts(word)(sense) mkString " "
  }

  def initializeCommunity(): Unit = {
    community_counts = Array.ofDim[Int](vocab_max_size, 10, 10)
  }

  def updateCommunity(word: Int, sense: Int, community: Int): Unit = {
    //    println(word)
    //    println(sense)
    //    println(community)
    try {
      community_counts(word)(sense)(community) += 1
    } catch {
