...
class MultiSenseSkipGramEmbeddingModel(override val opts: EmbeddingOpts) extends MultiSenseWordEmbeddingModel(opts) {
  ...
  override def process(doc: String): Int = {
    ...
    if (doc.stripLineEnd.split(' ').size == 1) {
      return 0
    }
    val community = Integer.valueOf(doc.stripLineEnd.split(' ')(0))
    ...
    for (senPosition <- 0 until senLength) {
      ...
      // LINAN: modification for rightSense would be here
      vocab.updateCommunity(currWord, rightSense, community)
